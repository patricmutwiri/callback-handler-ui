...
export function getSlugUserIndexKeys(user) {
  const keys = new Set()
  const id = user?.id ?? null
  const email = normalizeUserEmail(user?.email)

  if (id) {
    keys.add(`user_slugs:${id}`)
  }

  if (email) {
    keys.add(`user_slugs:${email}`)
  }
...
export function isSlugOwner(owner, sessionUser) {
  ...
  if (ownerEmail && sessionEmail) {
    return normalizeUserEmail(ownerEmail) === normalizeUserEmail(sessionEmail)
  }
